...
def mecanum_blend(x: float, y: float, w: float) -> List[float]:
    vx = clamp(apply_deadzone(square_inputs(x), constants["DEADZONE"]), -0.8, 0.8)      # forward/back
    vy = clamp(apply_deadzone(square_inputs(y), constants["DEADZONE"]), -0.8, 0.8)      # strafe right-left
    omega = clamp(apply_deadzone(square_inputs(w), constants["DEADZONE"]), -0.8, 0.8)     # turn
