...
def get_score(cycle_time, time_spent_cycling):
    ...
    # battery_points_per_second = TELEOP_BATTERY_POINTS / cycle_time
    # if battery_points_per_second <= 0.5:
    #     print("IT HAPPENED!!")

    score = battery_points + min(battery_points, kilojoules)

    return min(score, max(score, 20))
...
# actually graph
# plt.pcolormesh(SCORES, cmap=colormaps.get("veridis"))
ax1 = fig.add_subplot(projection="3d")
ax1.bar3d(cycle_times, times_spent_cycling, SCORES, 1, 1, 1)
# ax1.plot_surface(cycle_times, times_spent_cycling, NP_SCORES)
